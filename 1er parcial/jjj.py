...
def ordenar_jedi(jedis):
    ...
    while jedis:
        min_jedi = jedis[0]
        for jedi in jedis:
            if comparar_jedi(jedi, min_jedi) < 0:
                min_jedi = jedi
        jedis.remove(min_jedi)
        ordenar_jedis.append(min_jedi)
...
def show_jedi_info(jedis, jedi_names):
    for name in jedi_names:
        index = search(jedis, 'name', name)
        if index is not None:
            jedi_info = jedis[index]
            print("Información de", name)
            for key, value in jedi_info.items():
                print(f"{key}: {value}")
            print()

show_jedi_info(jedis, jedi_names)

print(" ")

# c) 
def padawans_masters(jedis, masters):
    padawan_list = []
...
padawan_list = padawans_masters(jedis, masters)
...
# d) 
def buscar_jedi(jedis, species):
    jedi_list = []
    for jedi in jedis:
        if jedi['species'] in species:
            jedi_list.append(jedi)
    return jedi_list

species_a_buscar = ['Human', 'Twi\'lek']   #Usé la barra(\) para que ' no indique el final del string

jedi_list = buscar_jedi(jedis, species_a_buscar)
...
# e)
def filtrar_nombre(jedis, letter):
    filtered_jedis = []
    for jedi in jedis:
        if jedi['name'].startswith(letter):
            filtered_jedis.append(jedi)
    return filtered_jedis

def show_jedis(jedis):
    print("Jedis:")
    for jedi in jedis:
        print(f"Name: {jedi['name']}")

jedis_con_A = filtrar_nombre(jedis, 'A')
show_jedis(jedis_con_A)
...
def jedi_with_multiple_colors(jedis):
    jedi_multiple_colors = []
    for jedi in jedis:
        if search('/', jedi['lightsaber_color']):
            jedi_multiple_colors.append(jedi)
    return jedi_multiple_colors
...
# Filtrar y mostrar Jedi que usaron sables de luz de más de un color
jedis_multiple_colors = jedi_with_multiple_colors(jedis)
show_jedis(jedis_multiple_colors)
