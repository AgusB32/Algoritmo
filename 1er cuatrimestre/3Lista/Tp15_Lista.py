...
# a) Obtener la cantidad de Pokémons de un determinado entrenador
def cantidad_pokemons(entrenadores, nombre_entrenador):
    # Busca el índice del entrenador por nombre
    index = search(entrenadores, 'nombre', nombre_entrenador)
    if index is not None:
        # Retorna la cantidad de Pokémons del entrenador encontrado
        return len(entrenadores[index]['pokemons'])
    return 0  # Si el entrenador no se encuentra, retorna 0
...
# b) Listar los entrenadores que hayan ganado más de tres torneos
def entrenadores_con_mas_de_tres_torneos(entrenadores):
    # Filtra y devuelve los nombres de entrenadores con más de 3 torneos ganados
    return [entrenador['nombre'] for entrenador in entrenadores if entrenador['torneos_ganados'] > 3]
...
# c) El Pokémon de mayor nivel del entrenador con mayor cantidad de torneos ganados
def obtener_mejor_entrenador(entrenadores):
    # Encuentra al entrenador con mayor cantidad de torneos ganados
    return max(entrenadores, key=lambda e: e['torneos_ganados'])

def obtener_mejor_pokemon(pokemons):
    # Encuentra el Pokémon de mayor nivel en una lista de Pokémons
    return max(pokemons, key=lambda p: p['nivel'])

def pokemon_mayor_nivel_del_mejor_entrenador(entrenadores):
    # Encuentra el mejor entrenador y luego su Pokémon de mayor nivel
    mejor_entrenador = obtener_mejor_entrenador(entrenadores)
    return obtener_mejor_pokemon(mejor_entrenador['pokemons'])
...
# e) Mostrar entrenadores cuyo porcentaje de batallas ganadas es mayor al 79%
def porcentaje_batallas_ganadas(entrenador):
    total_batallas = entrenador['batallas_ganadas'] + entrenador['batallas_perdidas']
    if total_batallas == 0:
        return 0
...
# f) Entrenadores que tengan Pokémon de tipo fuego/planta o agua/volador
def entrenadores_con_tipos_especificos(entrenadores):
    resultado = []
    for entrenador in entrenadores:
        tiene_fuego_planta = False
...
print("Entrenadores con Pokémon de tipo fuego/planta o agua/volador:", entrenadores_con_tipos_especificos(entrenadores))
...
# h) Determinar cuántos entrenadores tienen un Pokémon específico
def entrenadores_con_pokemon(entrenadores, nombre_pokemon):
    return len([entrenador for entrenador in entrenadores if any(pokemon['nombre'] == nombre_pokemon for pokemon in entrenador['pokemons'])])
...
print("Entrenadores con Pokémons repetidos:", entrenadores_con_pokemons_repetidos(entrenadores))

# j) Determinar los entrenadores que tienen uno de los siguientes Pokémon: Tyrantrum, Terrakion o Wingull
pokemons_especificos = ["Tyrantrum", "Terrakion", "Wingull"]
def entrenadores_con_pokemons_especificos(entrenadores, pokemons_especificos):
    return [entrenador['nombre'] for entrenador in entrenadores if any(pokemon['nombre'] in pokemons_especificos for pokemon in entrenador['pokemons'])]

print("Entrenadores que tienen a Tyrantrum, Terrakion o Wingull:", entrenadores_con_pokemons_especificos(entrenadores, pokemons_especificos))
